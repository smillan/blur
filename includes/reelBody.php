...
if ($_GET['name']==null) {
	  $currentShowreel = collection("showreels")->find()->sort(["created"=>1])->toArray();
	   $nombre = $currentShowreel[sizeof($currentShowreel) - 1]['titulo'] ;
}else{
	 $nombre = $_GET['name'] ;
 ...
 <?php 
 			//$nombre=$_GET['name']."html";
			 $markedInReel=array();
		 if ( $currentShowreel['directores']==null ||  $currentShowreel['directores']=="TODOS") {
            ...
            $galleries = (cockpit()->db->find('common/galleries')->toArray());
		 	...
		 	$currentShowreel['directores'];
 			$dirs=explode(",", $currentShowreel['directores']);
 			foreach ($dirs as $galleryName) {
 				echo $galleryName;
	            	$tempReel = gallery($galleryName );
...
<?php echo $currentShowreel['Descripcion'] ;?>
...
<div class="band ">
	...
	<!--  DIRECTOR CIRCLE NAME  -->
		<div id="dirCircle" class="one-third column dirPicSmall ">
				<div id='shape' class="  ">
					<div id="circleD"><div id="nombreDir"><?php echo $nombre;?></div></div>
				</div>
		</div>
	<!--  LIST  -->
	
		<?php $count=0; ?>
	<?php foreach ($reel as $itemReel ) :?>
		<!-- is it shown?-->
